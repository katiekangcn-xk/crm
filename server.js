...
async function sendMail(to, subject, text) {
  ...
  const tr = nodemailer.createTransport({ host: c.host, port: c.port || 465, secure: c.secure !== false, auth: { user: c.smtp_user, pass: c.smtp_pass } });
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === '/api/register' && req.method === 'POST') {
    ...
    const exist = await db('GET', 'users', { query: `?email=eq.${email}&select=id` });
    if (exist && exist.length) return json(res, 409, { error: 'email_exists', message: '该邮箱已注册' });
    const all = await db('GET', 'users', { query: '?select=id&limit=1' });
